...
def create_config(current_config_path):
    # Load base config
    with open(current_config_path, 'r') as f:
        config = json.load(f)
    
    # Determine port
    port = find_free_port(start_port=8080, end_port=9000)
    config['api_base_url'] = f"http://localhost:{port}/v1"
...
def main():
    ...
    parser.add_argument('--new_model_config', type=str, required=True,
    ...
    if config.get("provider_name", "local") == "local":
        return 1
